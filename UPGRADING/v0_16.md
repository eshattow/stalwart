...
> **Update embedded paths inside `config.json` and `export.json` for the new mount points.** The migration script writes the on-disk paths it found in the v0.15 deployment, which on the previous Docker image typically pointed at `/opt/stalwart/data` (and `/opt/stalwart/data/blobs` for the filesystem [BlobStore](https://stalw.art/docs/ref/object/blob-store)). The new image mounts persistent data at `/var/lib/stalwart` instead, so any path referencing the old location must be edited to point at the new one before the recovery container is started; otherwise the container exits with `Permission denied: /opt/stalwart/data` because UID `2000` cannot create that directory inside the container's filesystem. Two files need to be checked:
>
> - **`config.json`**: any embedded-store entry such as `RocksDb.path` or `SQLite.filePath` must be updated. For example, change `"path": "/opt/stalwart/data"` to `"path": "/var/lib/stalwart"`.
> - **`export.json`**: any filesystem `BlobStore` whose `basePath` references the old location must be edited the same way (`/opt/stalwart/data/blobs` → `/var/lib/stalwart/blobs`). Deployments that store blobs in S3, Azure Blob Storage, the database, or another non-filesystem backend can ignore this.
>
> A grep for `/opt/stalwart` against both files is the quickest way to confirm nothing was missed:
>
> ```bash
> $ grep -n /opt/stalwart /path/to/config.json /path/to/export.json
> ```
>
...
Creating a permanent administrator is also possible from [stalwart-cli](https://stalw.art/docs/category/command-line-interface/):

```bash
# Note: omit STALWART_PASSWORD if you want stalwart-cli to prompt for password
export STALWART_URL='http://[::1]:8080/'
export STALWART_USER='recoveryuser'
export STALWART_PASSWORD='recoverypass'

# Verify insecure HTTP connection to Stalwart server 
$ ./stalwart-cli --insecure describe

# Identify a Domain id to associate with the new admin account
$ ./stalwart-cli --insecure query Domain --fields 'id,name'

id  Domain Name
b   mydomain.tld

# Create the new admin account associated with Domain id (example: 'b')
$ ./stalwart-cli --insecure create account/user --field name=admin --field domainid=b

# Identify an Account id to associate with update of credentials and permissions
$ ./stalwart-cli --insecure query Account --fields 'id,name'

id  Username
e   admin
c   someuser

# Set a password credential associated with Account id (example: 'e')
# Required credential slot key name is any number 0 to 4*1024**3-1 for input ignored and replaced with 0
$ read -s -p "Stalwart new password: " stalwart_new_password ; echo
$ ./stalwart-cli update Account e --field 'credentials={"0":{"@type":"Password", "secret":"'"${stalwart_new_password}"'"}}'

# Set the Account permissions Administrator role
$ ./stalwart-cli update Account e --field '"roles"={"@type": "Admin"}'
```
